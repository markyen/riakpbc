...
ConnectionManager.prototype.connect = function (callback) {
    ...
    this.client.connect(this.port, this.host, function () {
        ...
        this.riak.startTls(function (err, reply) {
            if (err) {
                return callback();
            }

            return this._upgrade(callback);
        }.bind(this));
    }.bind(this));
};

// Private socket upgrade method, when the initial connect is
// run we send riak a starttls request. If that request succeeds
// we then defer to this method to upgrade the existing socket
// to a tls connection, and if credentials were supplied we
// automatically auth the connection.
ConnectionManager.prototype._upgrade = function (callback) {
    ...
    // remove the listeners we added so node core can handle the upgrade
    ...
    this.client = tls.connect({ socket: this.client, rejectUnauthorized: false }, function () {
        ...
        this.riak.auth(this.auth, callback);
    }.bind(this));
