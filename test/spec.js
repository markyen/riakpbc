...
exports.get = function (test) {
  client.get({ bucket: 'test', key: 'test' }, function (err, reply) {
    ...
    expect(reply.content[0].value).to.eql(expectedValue);
    test.done();
  });
};
...
exports.getKeysStream = function (test) {
  ...
  readStream.on('data', function (data) {
    test.ok(Array.isArray(data.keys));
    var len = data.keys.length;
    data.keys = data.keys.filter(function (key) {
      return (key.toString() === 'test' || key.toString() === 'test-large' || key.toString() === 'test-vclock' || key.toString() === 'test-put-index');
    });
    test.equal(data.keys.length, len);
  });
...
exports.mapredStreaming = function (test) {
  var request = {
    inputs: [['test', 'test']],
    query: [{
      map: {
  ...
  var streaming = true;
  var readStream = client.mapred({ request: JSON.stringify(request), content_type: 'application/json' }, streaming);
  var dataSpy = sinon.spy(dataHandler);
